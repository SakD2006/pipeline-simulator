 ...
 * - Branch parsing (FIXME: execution/prediction not implemented)
 * - Hazard types (RAW, structural)
 * - Dynamic instruction scheduling
 * - Performance metrics and visualization
 *
 * Pipeline stages: Fetch → Decode → Issue → Execute → WriteBack
 *
 * Compile: g++ -fopenmp pipeline_fixed.cpp -o pipeline && ./pipeline
 */
 ...
 private:
     struct RegInfo {
         bool busy;
         int writer_id;
         int ready_cycle; // Cycle when the register value is ready
     };
     vector<RegInfo> regs;
     const int num_registers;

 public:
     RegisterScoreboard(int num_regs = 32) : num_registers(num_regs), 
                                             regs(num_regs, {false, -1, -1}) {}

     bool isBusy(int reg, int current_cycle) {
         if (reg < 0 || reg >= num_registers) return false;
         // It's busy if it's marked busy AND the ready cycle is in the future
     ...
     void clearBusy(int reg) {
         if (reg >= 0 && reg < num_registers) {
             // We can just mark it as not busy.
             // The writer_id and ready_cycle are overwritten by the next writer.
             regs[reg].busy = false; 
         }
     }
 ...
 public:
     ...
     bool isAvailable(ExecUnit unit) {
         return available.count(unit) ? available[unit] > 0 : false;
     }
     ...
     // This should not be called every cycle!
     void reset() {
         available = capacity;
     }
 ...
 struct Statistics {
     ...
     int waw_hazards; // Not implemented
 ...
 // This runs in the sequential DECODE stage
 bool detectHazards(const Instruction& instr, PipelineState& state,
     ...
     // We check if the register is busy *at the current cycle*
     ...
     // This checks if a unit is available *at the time of decode*
     // The instruction will stall here until a unit is free.
 ...
 void displayPipeline(int cycle, const vector<Instruction>& instrs,
     ...
     // This critical section prevents jumbled output from multiple threads
     #pragma omp critical(Display)
     {
         ...
         cout << "├─────────────────────────────────────────────────┤" << endl;
         // Show unit status *after* all allocations/releases for the cycle
 ...
 int main(int argc, char* argv[]) {
     ...
     cout << "║  5-Stage Superscalar Out-of-Order Completion  ║" << endl;
     cout << "╠════════════════════════════════════════════════╣" << endl;
     ...
     int cycle = 0;
     int completed = 0;
     const int MAX_CYCLES = 500; // Increased max cycles for long latencies
     ...
     while (completed < instructions.size() && cycle < MAX_CYCLES) {
         ...
         // -----------------------------------------------------------------
         // CRITICAL FIX #1: REMOVED `exec_units.reset()` from here
         // We must NOT reset units every cycle. They are allocated in ISSUE
         // and released in WRITEBACK.
         // -----------------------------------------------------------------

         // WriteBack stage (parallel)
         // Releases resources and marks instructions as complete
         #pragma omp parallel for schedule(dynamic)
         for (int i = 0; i < instructions.size(); i++) {
             if (states[i].current_stage == WRITEBACK) {
                 // Clear the destination register from the scoreboard
                 scoreboard.clearBusy(instructions[i].dest);
                 
                 // -----------------------------------------------------------------
                 // CRITICAL FIX #2: Add thread-safety for resource release
                 // Multiple threads can't call release() on the same unit
                 // simultaneously without protection.
                 // -----------------------------------------------------------------
         ...
         // Instructions stay here for their specified latency
         #pragma omp parallel for schedule(dynamic)
         for (int i = 0; i < instructions.size(); i++) {
         ...
         // Tries to allocate an execution unit
         // Must be sequential to avoid two instructions grabbing the same unit
         for (int i = 0; i < instructions.size(); i++) {
             if (states[i].current_stage == ISSUE) {
                 ExecUnit unit = getExecUnit(instructions[i].opcode);

                 // Attempt to allocate the unit
                 if (exec_units.allocate(unit)) {
                     ...
                     // It will be ready after (current cycle + latency)
                 ...
                 // If allocation fails, it stalls in ISSUE and will try again next cycle
             }
         }

         // Decode stage (sequential for hazard detection)
         // Checks for RAW and Structural hazards
         // Must be sequential to check scoreboard/units in a defined order
         for (int i = 0; i < instructions.size(); i++) {
             if (states[i].current_stage == DECODE) {
                 // Only advance if no hazards are detected
                 if (detectHazards(instructions[i], states[i], scoreboard,
                                  exec_units, cycle, stats)) {
                     states[i].current_stage = ISSUE;
                 }
                 // If hazards detected, it stalls in DECODE
             }
         }

         // Fetch stage (parallel)
         // Fetches all instructions that are IDLE
         // This simulates an "infinite" fetch buffer and width
         #pragma omp parallel for schedule(dynamic)
         for (int i = 0; i < instructions.size(); i++) {
             if (states[i].current_stage == FETCH) {
                 ...
                 // For this simulation, we fetch all instructions at once
                 // A more complex sim might have a fetch window/PC
     ...
     if (cycle >= MAX_CYCLES) {
         cout << "WARNING: Simulation timed out at " << MAX_CYCLES << " cycles." << endl;
         cout << "         " << completed << "/" << instructions.size() 
              << " instructions completed." << endl;
     }
     ...
     cout << "─────────────────────────────────────────────────" << endl;
     cout << "ID  | Opcode | Issue | Complete | Total Cycles" << endl;
     cout << "────|────────|───────|──────────|─────────────" << endl;
     for (size_t i = 0; i < instructions.size(); i++) {
         cout << "I" << setw(2) << instructions[i].id << " | "
              << setw(6) << left << opcodeToString(instructions[i].opcode) << " | "
              << setw(5) << states[i].issue_cycle << " | "
              << setw(8) << states[i].complete_cycle << " | "
